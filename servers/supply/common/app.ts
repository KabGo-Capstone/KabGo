...
class Application {
    ...
    private setup() {
        ...
        this.app.get('/status', (req, res) => {
            return res.json({ server: this.appName.replace(/\[(.*)\]/, '$1'), status: '200 - OK', message: 'Server is running ...' });
        });
    ...
    public run(callback: () => void = () => {}): Server {
        ...
        const server: Server = this.app.listen(availablePort, async () => {
            try {
                Logger.info(chalk.green(`Server is running on port ${chalk.cyan(availablePort)}`));
                ...
                io.on('connection', (socket: Socket) => {                   
                    Logger.info(`${chalk.green('Connected')} - Socket ID: ${socket.id}`);
